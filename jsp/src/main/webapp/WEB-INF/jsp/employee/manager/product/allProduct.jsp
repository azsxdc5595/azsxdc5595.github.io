	...
	<!-- 分頁信息 -->
            <div style="text-align: center; margin-top: 10px;">
                    ...
                    <a class="<%=pageInfo.isHasPreviousPage() ? "" : "disabled"%>"
                        href="/product/product2?page=<%=pageInfo.getPrePage()%>&size=<%=pageInfo.getPageSize()%>">&laquo; 上一頁</a>

                    <!-- 顯示分頁數字 -->
                    <% for (int pageNum = 1; pageNum <= totalPages; pageNum++) { %>
                        <a class="<%=pageNum == currentPage ? "active" : ""%>"
                            href="/product/product2?page=<%=pageNum%>&size=<%=pageInfo.getPageSize()%>"><%=pageNum%></a>
                    <% } %>

                    <!-- 下一頁 -->
                    <a class="<%=pageInfo.isHasNextPage() ? "" : "disabled"%>"
                        href="/product/product2?page=<%=pageInfo.getNextPage()%>&size=<%=pageInfo.getPageSize()%>">下一頁 &raquo;</a>
            </div>
	<a href="/member/">回上頁</a>
</div>
</div>
